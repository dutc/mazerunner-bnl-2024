...
class Maze:
    ...
    def from_file(cls, filename):
        with open(filename) as f:
            lines = (
                (ln[: ln.index("#")] if "#" in ln else ln).rstrip("\n")[::2] for ln in f
            )
...
def overlay(lines, symbols: dict[tuple[int, int], str]):
    for rownum, row in enumerate(lines):
        yield [symbols.get((rownum, colnum), col) for colnum, col in enumerate(row)]
        ...
        async def __call__(self):
            ...
            while True:
                if self.power is AgentPower.On:
                    if (
                        self.power_cycle is not None
                        and self.total_ticks % self.power_cycle == 0
                    ):
                    ...
                    match self.state:
                        case AgentState.Moving:
                            ...
                            if not (
                                0 <= new_loc[0] < maze.shape[0]
                                and 0 <= new_loc[-1] < maze.shape[-1]
                            ):
                                pass
                            elif maze[old_loc] is Tile.Exit:
                                self.location = old_loc
                            elif (
                                maze[new_loc] is Tile.Floor
                                or maze[new_loc] is Tile.Exit
                            ):
                ...
                tick = (
                    rnd.uniform(0.5 * self.tick, 1.5 * self.tick)
                    if errors
                    else self.tick
                )
                logger.debug(
                    ...
                    self.power_cycle,
                    self.location,
                    next(self._hold).angle,
                    self.state,
                    self.power,
                    maze[self.location] is Tile.Exit,
                )
                self.total_ticks += 1
        ...
        def handler(self):
            async def handler(reader, writer):
                while True:
                    ...
                    resp = Response.Error()
                    if errors:
                        condition = rnd.choices(
                            ["stall", "drop", "disconnect", "error", "noerror"],
                            weights=[5, 5, 5, 5, 80],
                            k=1,
                        )[0]
                    ...
                    match req:
                        ...
                        case Request.StopTurn() if self.power is AgentPower.On:
                            if (
                                self.state is AgentState.TurningLeft
                                or self.state is AgentState.TurningRight
                            ):
                                self.state = None
                                resp = Response.TurningStop()
                        case Request.CheckMove() if self.power is AgentPower.On:
                            if self.state is AgentState.Moving:
                                resp = Response.MovingState(
                                    abs(
                                        array(self.prev_location) - array(self.location)
                                    ).sum()
                                )
                        ...
                        case Request.FrontSensor() if self.power is AgentPower.On:
                            ...
                            if (
                                0 <= sensor_loc[0] < self.maze.maze.shape[0]
                                and 0 <= sensor_loc[-1] < self.maze.maze.shape[-1]
                            ):
                                resp = (
                                    Response.Wall()
                                    if self.maze.maze[sensor_loc] is Tile.Wall
                                    else Response.NoWall()
                                )
                        ...
                        case Request.ExitSensor() if self.power is AgentPower.On:
                            resp = (
                                Message.Exit()
                                if self.maze.maze[self.location] is Tile.Exit
                                else Message.NoExit()
                            )
                    writer.write(resp.serialize())
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="run mazerunner agent standalone",
)
parser.add_argument(
    "--errors", action="store_true", default=False, help="enable errors"
)
parser.add_argument(
    "--host", type=str, default="127.0.0.1", help="TCP host for mazerunner agent"
)
parser.add_argument(
    "--port", type=int, default=8855, help="TCP port for mazerunner agent"
)
...
parser.add_argument(
    "--power-cycle", type=int, default=None, help="power cycle frequency"
)
parser.add_argument(
